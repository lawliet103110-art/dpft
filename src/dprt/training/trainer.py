...
class CentralizedTrainer():
    ...
    def validate_one_epoch(self, epoch: int, model: torch.nn.Module, data_loader: Iterable,
        ...
        # Return all metrics (loss, mAP, etc.)
        return scalars
    ...
    def train(self, model: torch.nn.Module, data_loader: Iterable, val_loader: Iterable = None,
        ...
        checkpoint_dir = osp.join(dst, timestamp, 'checkpoints')
        ...
        import csv
        metrics_file = osp.join(dst, timestamp, 'metrics.csv')
        csv_file = open(metrics_file, 'w', newline='')
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(['epoch', 'loss', 'mAP'])  # Header
        print(f"Metrics will be saved to: {metrics_file}")

        # Track best mAP for saving best checkpoint
        best_mAP = -float('inf')
        ...
        for epoch in tbar:
            ...
            epoch_mAP = result.get('mAP', 0.0)
            csv_writer.writerow([epoch, epoch_loss, epoch_mAP])
            csv_file.flush()  # Ensure data is written immediately

            # Save checkpoint
            path = osp.join(checkpoint_dir,
                            f"{timestamp}_checkpoint_{str(epoch).zfill(4)}.pt")
            torch.save(model, path)

            # Save best checkpoint if current mAP is higher
            if epoch_mAP > best_mAP:
                best_mAP = epoch_mAP

                # Delete old best checkpoint if exists
                if best_checkpoint_path is not None and osp.exists(best_checkpoint_path):
                    os.remove(best_checkpoint_path)

                # Save new best checkpoint with special naming
                best_checkpoint_path = osp.join(checkpoint_dir,
                                                f"{timestamp}_best_mAP_{epoch_mAP:.4f}_epoch_{str(epoch).zfill(4)}.pt")
                torch.save(model, best_checkpoint_path)
                print(f"\n✓ New best mAP: {epoch_mAP:.4f} at epoch {epoch}")

            # Clean up old regular checkpoints (keep last 3 + best)
            all_regular_checkpoints = sorted(glob.glob(osp.join(checkpoint_dir, f"{timestamp}_checkpoint_*.pt")))
            if len(all_regular_checkpoints) > 3:
                for old_checkpoint in all_regular_checkpoints[:-3]:

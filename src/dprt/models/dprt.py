...
class DPRT(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> OrderedDict[str, torch.Tensor]:
        ...
        # 如果没有活跃模态，调用querent后还需要通过head确保输出包含所有必要键
        if not active_inputs:
            # 获取querent输出作为参考点
            ref_points = self.querent(batch)
            # 创建一个足够大的特征张量来通过head产生完整输出
            # 假设batch_size为ref_points['center'].shape[0]，特征维度为16
            batch_size = ref_points['center'].shape[0]
            num_queries = ref_points['center'].shape[1]
            dummy_features = torch.zeros(batch_size, num_queries, 16, device=next(self.parameters()).device)
            # 确保ref参数格式正确，包含center键
            ref_input = {'center': ref_points['center']}
            # 通过head模块生成包含所有必要键的输出
            return self.head(dummy_features, ref_input)

        # Get input data shapes in channel last format (B, H, W, C)
        shapes = {input: batch[f"{input}_shape"] for input in active_inputs}

        # Feature extraction
        features = {input: self.backbones[input](batch[input]) for input in active_inputs}
        ...
        # Feature alignment
        features = {input: self.necks[input](features[input]) for input in active_inputs}

        # Positional embedding
        features = {input: self.embeddings[input](features[input]) for input in active_inputs}
        ...
        # Sensor fusion
        out = self.fuser(
            batch=[features[input] for input in active_inputs],
            shape=[shapes[input][:, :2] for input in active_inputs],
            projection=self._get_projetions(active_inputs, batch),
            out=out
        )
